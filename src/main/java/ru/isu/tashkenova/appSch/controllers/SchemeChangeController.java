...
//                            ids.add(teacherId);
        ...
        for (Schedule s: contentSch) {
            if (s.subjectId!=-1) {
                ...
                labels[s.numberOfTheLesson][s.getStudentClassId()+1].setCabinetId(s.cabinetId);
                //gridpane.add(labels[s.numberOfTheLesson][s.getStudentClassId()+1], s.getStudentClassId()+1, s.numberOfTheLesson);
            }
